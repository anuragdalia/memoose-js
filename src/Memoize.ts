...
import {CacheKey, CacheProvider, Pipeline, TTL} from "./adapters/base";
...
export type MemoizeConfig<T = any> = {
    functionName?: string;
...
export class Memoize<T = any> {
    ...
    private readonly multiExecOverride: (...argss: Args[]) => Promise<T[]>;
     ...
     */
    constructor(func: Function, ttl: number = -1, options: MemoizeConfig<T> = {}) {
        this.func = func;
        this.ttl = ttl;
    ...
    private isCacheMiss(obj?: CachedItem | null): obj is null {
        return obj === null || obj === undefined
    }

    private processedObjFromCache(obj: CachedItem): Promise<T> {
        function processStr(obj: string) {
    ...
    private processObjForCache(obj: any, rejection: boolean = false) {
        if (this.storeAsObj) {
    ...
    private pipeline(this: Memoize<T>): Pipeline<CachedItem> {
    ...
    public cacheKey(this: Memoize<T>, ...args: Args): string {
        return this.cacheKeyGenerator.for(...args);
    };

    //function single call

    private async executeAndCache(cache_key: CacheKey, ...args: Args) {
        try {
    ...
    private async saveToCache(resp_val_original: T | any, cache_key: CacheKey, ttl: TTL, rejection: boolean = false) {
        if (ttl <= 0) ttl = -1;
    ...
    public async exec(this: Memoize<T>, ...args: Args): Promise<T> {
        return this.func.apply(null, args)
    };

    public async call(this: Memoize<T>, ...args: Args): Promise<T> {
    ...
    public async update(this: Memoize<T>, ...args: Args): Promise<T> {
    ...
    public async demoize(this: Memoize<T>, ...args: Args): Promise<number> {
        const cache_key = this.cacheKeyGenerator.for(...args);
        return this.cache.del(cache_key);
    };

    public async refresh(this: Memoize<T>, ...args: Args): Promise<any> {
    ...
    public async setExp(this: Memoize<T>, ...args: Args) {
        let newTTL = args.pop();
        const cache_key = this.cacheKey(...args);
    ...
    private async multiExecuteAndCache(this: Memoize<T>, ...cache_key_and_args_s: [CacheKey, Args][]): Promise<T[]> {
        const argssToExec = cache_key_and_args_s.map(([cacheKey, args]) => args)
        const newRespGen = await this.multiExec(...argssToExec)

        const argsToSaveToCache: [T, CacheKey, TTL, boolean][] = []
    ...
    private async multiSaveToCache(this: Memoize<T>, ...argss: [T, CacheKey, TTL, boolean][]) {
    ...
    public async multiExec(this: Memoize<T>, ...argss: Args[]) {
        return this.multiExecOverride.apply(null, argss);
    };

    public async multiCall(this: Memoize<T>, ...argss: Args[]): Promise<T[]> {
        if (argss.length === 0)
            return Promise.resolve([])
        ...
        const hits: [number, [CacheKey, Args], T][] = []
        ...
        const newResps: T[] = await this.multiExecuteAndCache(...arggsToExecAndCache)
