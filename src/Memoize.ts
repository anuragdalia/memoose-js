...
export class Memoize<T> {
    ...
    private cacheKeyGenerator: CacheKey;
    private readonly name: string;
    private multiExecOverride: (argss: any[][]) => Array<T>;
    ...
    constructor(func: Function, ttl: number = -1, options: {
        ...
        inMemory?: boolean;
        multiExecOverride?: (argss: any[][]) => Array<T>
    } = {
        functionName: func.name,
        argsOrderVain: false,
        inMemory: false,
        multiExecOverride: undefined
    }) {
        this.func = func;
        this.ttl = ttl;
        ...
        this.multiExecOverride = options.multiExecOverride || ((argss: any[][]) => argss.map(args => func(...args)))

        if (this.functionName === "")
            throw new Error("Anonymous functions can not be memoized. Either pass functionName in options");

        this.cacheKeyGenerator = new CacheKey(this.functionName, this.argsOrderVain);
        ...
        stats[this.inMemory ? "memory" : "redis"][this.functionName] = stats[this.inMemory ? "memory" : "redis"][this.functionName] || {
            hit: 0,
            miss: 0,
            total: 0,
            adv_stats: {}
    ...
    public refresh(...args: any[]): Promise<T> {
        ...
        const cache_key = this.cacheKeyGenerator.for(argsToGenerateKey);
        return this.cache.del(cache_key)
    ...
    public multiCall(this: Memoize<T>, ...args: any[]): Promise<T[]> {
        ...
        const cache_keys = args.map(arg => this.cacheKeyGenerator.for([].concat(arg)));
        const start = Date.now();
